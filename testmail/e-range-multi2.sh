...
# also send a mail to a wrong recipient
################################################################################ 
...
#	expect "250-STARTTLS"
	...
	for {set sz $sz1} {$sz <= $sz2} {incr sz} {
		...
		send "mail from: <yuri.epstein@rambler.ru>\r"
		expect "250 2.1.0 <yuri.epstein@rambler.ru> ok"
		send "rcpt to: <sales@25volt.ru>\r"
		expect "250 2.1.5"	# resmtp doesn't check recipient by itself
		send "data\r"
		expect "354 Enter mail data, end with <CRLF>.<CRLF>"
		send "From: <yuri.epstein@rambler.ru>\r"
		send "To: <sales@25volt.ru>\r"
		send "Subject: $sz bad\r\r"
		send "$fdata"
		send "\n\n\r\n\\\\r\r\r\rr\r"
		send "\r.\r"
...
if {[llength $argv] != 2} {
	puts "Usage: e-range-multi2.sh <size1> <size2>"
