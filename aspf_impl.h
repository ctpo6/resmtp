        ...
        for (int m = 0; m < r->num_mech; m++, mech=SPF_mech_next(mech)) {
            ...
            switch (mech->mech_type) {
                ...
                case MECH_PTR:
                {
                    ...
                    collect_spf_dns_data_ptr(
                                collect_state(st.shared_state,
                                              util::rev_order_av4_str(boost::asio::ip::address_v4(ipv4),
                                                                      "in-addr.arpa")),
                                handle);
                    break;
                }
...
{
    string name, domain;
    if (!p.from.empty() && util::parse_email(p.from, name, domain))
        p.domain = domain;
